...
\begin{lemma}[Characterization of Conditional Expectation Kernels]
    \label{def:cond-exp-ker-char}
    \uses{def:cond-exp-ker, def:proper-kernel}
    \lean{ProbabilityTheory.Kernel.IsProper, ProbabilityTheory.Kernel.Condexp}
    \leanok

    Let $(X,\mathcal{X})$ be a measurable space, let $\mathcal{B}$ be a sub $\sigma$-algebra of $\mathcal{X}$ and let $\mu\in\mathfrak{M}(X,\mathcal{X})$. Then, $\pi\in\text{Ker}_{\mathcal{B},\mathcal{X}}$ is a proper conditional expectation kernel iff $\mu\circ_k \pi=\mu$.
\end{lemma}
