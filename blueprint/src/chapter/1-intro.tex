...
\begin{proof}
    ...
    This is an application of the standard machine for the Lebesge integral. For all $\Lambda \in \Finset(S)$, $A \in \mathcal E^S$, $B \in \mathcal{F}_{S\setminus\Lambda}$, $\eta : S \to E$, we want to prove
    ...
    which is true for $\rho_\Lambda = 1_C$ by properness of $\gamma_\Lambda$ on $A \inter C$ and $B$, and the standard machine goes through.
\end{proof}
