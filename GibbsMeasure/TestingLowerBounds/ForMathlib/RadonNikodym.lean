...
lemma Measure.absolutelyContinuous_of_compProd {μ ν : Measure α} {κ η : Kernel α γ}
  ...
  swap; · exact κ.measurable_coe MeasurableSet.univ
  by_contra hμs
...
lemma eq_rnDeriv_measure [IsFiniteKernel η] (h : κ = withDensity η f + ξ)
...
lemma eq_rnDeriv [IsFiniteKernel κ] [IsFiniteKernel η] (h : κ = withDensity η f + ξ)
    (hf : Measurable (Function.uncurry f)) (hξ : ∀ a, ξ a ⟂ₘ η a) (a : α) :
    f a =ᵐ[η a] rnDeriv κ η a :=
  (eq_rnDeriv_measure h hf hξ a).trans (rnDeriv_eq_rnDeriv_measure _ _ a).symm

lemma eq_singularPart [IsFiniteKernel κ] [IsFiniteKernel η] (h : κ = withDensity η f + ξ)
    (hf : Measurable (Function.uncurry f)) (hξ : ∀ a, ξ a ⟂ₘ η a) (a : α) :
...
lemma measurable_singularPart (κ η : Kernel α γ) [IsFiniteKernel κ] [IsFiniteKernel η] :
  ...
  simp_rw [← singularPart_eq_singularPart_measure, singularPart_def κ η]
...
lemma withDensity_rnDeriv_eq
    ...
    withDensity η (rnDeriv κ η) a = κ a := by
  rw [η.withDensity_apply]
  swap; · exact measurable_rnDeriv _ _
  have h_ae := rnDeriv_eq_rnDeriv_measure κ η a
...
lemma rnDeriv_withDensity
    ...
    rnDeriv (withDensity κ f) κ a =ᵐ[κ a] f a := by
  have h_ae := rnDeriv_eq_rnDeriv_measure (withDensity κ f) κ a
...
lemma withDensity_rnDeriv_le (κ η : Kernel α γ) [IsFiniteKernel κ] [IsFiniteKernel η] (a : α) :
  ...
  rw [setLIntegral_congr_fun hs
    ((rnDeriv_eq_rnDeriv_measure κ η a).mono (fun x hx _ ↦ hx)), ← withDensity_apply _ hs]
...
lemma Measure.absolutelyContinuous_kernel_of_compProd {μ ν : Measure α} {κ η : Kernel α γ}
    ...
    (h : μ ⊗ₘ κ ≪ ν ⊗ₘ η) :
    ∀ᵐ a ∂μ, κ a ≪ η a := by
  suffices ∀ᵐ a ∂μ, singularPart κ η a = 0 by
...
omit [MeasurableSpace.CountableOrCountablyGenerated α γ] in
lemma absolutelyContinuous_compProd_iff {β : Type*} [MeasurableSpace β]
  ...
  simp_rw [compProd_apply_eq_compProd_snd', Measure.absolutelyContinuous_compProd_iff, snd'_apply]
